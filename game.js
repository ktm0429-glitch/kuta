...
function drawHomeBG() {
	...
	// upper foul lines: a shallow "tent" peaking above the mound, fading into
	// plain grass before reaching home plate (matches the source art, which
	// does not draw one continuous line the whole way)
	ctx.strokeStyle = '#fff';
	ctx.lineWidth = 2.5;
	ctx.beginPath();
	ctx.moveTo(0, 80);
	ctx.lineTo(base_center(), 68);
	ctx.lineTo(STAGE_W, 80);
	ctx.stroke();

	// lower foul lines, resuming near the batter boxes
	ctx.beginPath();
	ctx.moveTo(0, 152);
	ctx.lineTo(base_center() - 92, 175);
	ctx.moveTo(STAGE_W, 152);
	ctx.lineTo(base_center() + 92, 175);
	ctx.stroke();
