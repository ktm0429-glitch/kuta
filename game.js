...
// Wall height tapers across the width so it reads as receding toward centre
// field (a flat wall looked artificial). The bottom edge - the part the
// home-run judgement actually cares about - is always exactly fenceYAt(),
// unaffected by this taper.
function wallHAt(dir, x) {
	if (dir === 'center') return 20;
	const t = x / STAGE_W; // 0..1 across the screen
	return dir === 'left' ? (52 - 36 * t) : (16 + 36 * t);
}
...
function drawOutfieldBG(dir) {
	...
	if (dir === 'left' || dir === 'right') {
		const poleX = dir === 'left' ? 0 : STAGE_W;
		...
		ctx.lineWidth = 3;
		ctx.beginPath();
		ctx.moveTo(dir === 'left' ? STAGE_W : 0, STAGE_H);
